...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="whole",
        choices=["whole", "text", "image", "image-fc", "image-fc-fixed", "fc", "ffn_out", "image_ffn_out", "full_ffn"],
    ...
    parser.add_argument("--few_shot", type=int, default=-1, help="Number of shots per class; <=0 uses the full training split.")
    parser.add_argument("--random-mask-ratio", type=float, default=0.0, help="Randomly freeze this fraction of trainable parameter elements by masking gradients.")
    parser.add_argument("--random-mask-seed", type=int, default=None, help="Seed for the random gradient mask; defaults to --seed.")
    parser.add_argument("--eval-all-labels", action="store_true", help="Evaluate with one fixed global label space across all MTIL split tasks.")
    parser.add_argument("--post-train-eval", action="store_true", help="Evaluate the in-memory fine-tuned model once after training finishes.")
    parser.add_argument("--global-label-datasets", default=None, type=lambda x: x.split(","), help="Comma-separated dataset names used to build the fixed global label space.")
    parser.add_argument("--merged-kv-output", default=None, help="When set, append this task's trained/base KV stats into one merged KV file after training.")
    parser.add_argument("--merged-kv-datasets", default=None, type=lambda x: x.split(","), help="Comma-separated dataset order used for merged KV weighting metadata.")
    parser.add_argument("--target-weights", "--weights1", dest="target_weights", default="", help="Manual per-dataset weights for trained target KV stats, as name:weight,... .")
    parser.add_argument("--anchor-weights", "--weights0", dest="anchor_weights", default=None, help="Manual per-dataset weights for base anchor KV stats; defaults to target weights when omitted.")
    parser.add_argument("--scale-reference", choices=("global", "first", "none"), default="global", help="Merged KV balancing mode; global scales each task to fixed kk reference values before manual weights.")
    parser.add_argument("--target-scale-reference-kk", type=float, default=None, help="Fixed target kk_sum reference used when --scale-reference=global.")
    parser.add_argument("--anchor-scale-reference-kk", type=float, default=None, help="Fixed anchor kk_sum reference used when --scale-reference=global.")
